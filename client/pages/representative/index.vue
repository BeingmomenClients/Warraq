<template>
  ...
  >
    ...
    >
      <template #bookstores-data="{ row }">
        <div class="grid grid-cols-1">
          <UButton
            v-for="store in row.bookstores"
            :key="store._id"
            variant="link"
            :to="`/bookstore/${store._id}/purchase-invoices`"
          >
            {{ store.name }}
          </UButton>
        </div>
...
const customItems = {
  items: (row: any) => [
    {
      label: "فواتير بيع المندوب",
      icon: "i-ic-outline-playlist-add",
      click: () => {
        navigateTo(`/representative/${row.id}/purchase-invoices`);
      },
    },
    {
      label: "تحصيل مبلغ من المندوب",
      icon: "i-heroicons-squares-plus",
      click: () => {
        navigateTo(`/representative/${row.id}/collect-an-amount`);
      },
    },
  ],
};
