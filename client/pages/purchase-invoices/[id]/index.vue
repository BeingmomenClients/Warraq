...
const isUpdatingBookIds = ref(false);

if (isEditMode.value) {
  watch(
    () => initialState.bookIds,
    async (newValue) => {
      if (!isUpdatingBookIds.value) {
        await newValue.forEach((book: any, i: number) => {
          updateTotal(i, book.book[0].availableQuantity, book.book[0].price);
        });

        isUpdatingBookIds.value = await true;
        initialState.bookIds = await newValue.map((book: any) => ({
          bookId: book.book[0]._id,
          price: book.price,
          count: book.count,
          total: book.total,
          maxCount: book.book[0].availableQuantity,
          minPrice: book.book[0].price,
        }));
      }
    },
    { deep: true }
