const sendErrorDev = (err, res) => {
  ...
  } else if (err.codeName === 'WriteConflict') {
    errorsArr.error = [
      'Resource is read-only or being updated by another transaction'
    ];
  } else {
    if (!err.isHandled) {
      ...
      errorsArr[key] = [`[ ${err.keyValue[key]} ] موجود مسبقا`];
