...
exports.getAll = (Model, options) =>
  catchAsync(async (req, res, next) => { 
    ...
    const { popOptions = [], nextStep = false, stopRes = true, optFilter = {}, optSort = {}} = options || {};
    ...
    let filter = { role: { $ne: 'dev' }, ...mergeFilter, ...optFilter };


    const features = new APIFeatures(Model.find(filter), {...req.query, ...optSort})
      .filter()
      .sort()
      .limitFields()
      .search();
...
exports.getAllRepresentativeNoPagination = (Model, popOptions = []) =>
  catchAsync(async (req, res, next) => {
    const doc = await Model.find({ role: 'representative' })
      .select('name')
      .populate(popOptions);
